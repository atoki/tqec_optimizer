                ...
                for edge in self._graph.edge_list:
                    if edge.z == z and edge.dir != "Z":
                        if self.__can_move(edge):
                            ...
                            slide_edge_list.append(edge)

                self.__move(press_edge_list)
                self.__slide(slide_edge_list)
                press_edge_list.clear()
                slide_edge_list.clear()

    def __move(self, edge_list):
        for edge in edge_list:
            node1, node2 = edge.node1, edge.node2
            id_, type = node1.edge_list[0].id, node1.type

            # create node
            new_node1, exist1 = self.__new_node(type, node1.x, node1.y, node1.z - 2)
            new_node2, exist2 = self.__new_node(type, node2.x, node2.y, node2.z - 2)
